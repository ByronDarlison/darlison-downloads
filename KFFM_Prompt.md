...
0. The chosen layout matches one of the published templates (3-function single row, 4-function 2x2, 5-function 3+2). Every `<rect>` x-coordinate is one of the values listed in that template's `key_boxes` JSON list. The viewBox dimensions match the template's stated `viewbox` exactly. If any box uses a custom x not in the template, you must regenerate the SVG from the template before continuing the checklist. **Cite the template by name in your narration:** `✓ Check 0: Using template '4-functions-2x2' (viewBox 850x720, box positions [(40,80), (450,80), (40,280), (450,280)]).`
...
14. Every connector has a widget label `<text>` in `class="widget"` near it.
...
16. Profit/X text appears as one line in `class="profitx-svg"` (NOT inline `style=`), format `Profit / X = Profit / [denominator]` OR `Profit / X = Profit per [denominator]`, with no dollar value.
17. Dashed separator (`stroke-dasharray="4,4"`, `stroke="#bbb"`) appears immediately below the Profit/X line.
18. Lead-to-cash block appears below the supporting function row, titled `Time to money` in `class="l2c-head"` (NOT inline `style=`), with one `<text class="l2c-text">` (NOT inline `style=`) per path.
18a. **Each lead-to-cash text line uses the literal format `[Path name] lead-to-cash: [N] days`** — the path name comes first, then the literal phrase `lead-to-cash:` (lowercase, hyphenated), then the integer day count, then the literal word `days`. Examples that pass: `New customer lead-to-cash: 80 days`, `Expansion lead-to-cash: 14 days`, `Inbound lead-to-cash: 26 days`. Examples that FAIL the check: `New customer path: Marketing + Sales + Finance = ~16 weeks` (uses `path:` not `lead-to-cash:`, sums functions instead of stating the time, uses weeks not days), `Standard path is about 26 days end to end` (paragraph form, missing `lead-to-cash:`), `Lead-to-cash time: 80` (missing `days` suffix). The structural check rejects any L1 SVG whose `Time to money` block does not contain at least one `<text>` matching the regex `lead-to-cash:\s*\d+\s*days`. Cite parity in narration: `✓ Check 18a: Lead-to-cash lines = ['New customer lead-to-cash: 80 days', 'Expansion lead-to-cash: 14 days']. All N lines match required format.`
