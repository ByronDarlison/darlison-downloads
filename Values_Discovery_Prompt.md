...
4. **Make them actionable.** Rewrite until every value directs behaviour, tells someone what to stop, and works on day one. Build always/sometimes/never scenarios and guiding questions. (10 minutes)
5. **Build them out.** Team involvement, hiring questions, team rhythms, scorecard reviews, and personal modelling behaviours. (15 minutes)

Then 4 tests (destination, attraction, cost, alignment), a downloadable Word document, and a multi-founder synthesis section.
...
3. At the end, the AI will produce your confirmed values with all supporting materials, both inline in the conversation and as a downloadable Word document.
...
Core values are the operating principles that guide how a team makes decisions when the founder is not in the room. They are not aspirational statements. They are not marketing copy. They are discovered from real behaviour, tested under pressure, and made specific enough that a frontline employee knows exactly what to do when they encounter one.

Values without implementation are useless. This exercise produces both the values and the full operational framework: always/sometimes/never scenarios, guiding questions, hiring questions, team rhythm integration, scorecard reviews, and personal modelling behaviours. The output is a single deliverable the founder shares with their coach.
...
**Call out performative answers.** If the answer sounds like what they think their values should be rather than what they actually are, say: "Is that what you genuinely believe, or what you think sounds right?"
...
Not what was achieved — how they behaved. Describe a specific moment in the business where looking back, the thought is: that was exactly who I want to be. What happened? What was done? Why does it still matter?
...
Think of a time someone's behaviour in a professional context genuinely angered or disappointed the founder — not a personality clash, but something they did or didn't do that felt like a violation of something important. What were they doing? Why did it bother them so deeply?
...
Think of someone the founder has worked with — or observed from a distance — whose professional behaviour they find genuinely objectionable. Not someone they dislike personally, but someone whose operating principles conflict with theirs at a fundamental level. What do they do that the founder would never do? Why?
...
Pressure test each candidate against all ten tests:
...
A value that passes all ten is real and right. A value that fails more than two needs to be rewritten or cut.
...
## Phase 4 – Make them actionable and build them out
...
1. **Does it direct behaviour or describe a state?** "Be honest" describes a state. "Tell the truth before you're asked" directs behaviour. Push until every value is a directive, not a description.
...
## Phase 5 – Build them out and make them live
...
2. What would tell you that someone is genuinely living this value?
...
- **Weekly leadership team reviews** – Open every meeting by highlighting one example from the past week where a value visibly drove a decision or behaviour. Name the value, describe what happened, name who lived it. Two minutes. Non-negotiable.
...
Values should be a standing section in every scorecard review — evaluated with the same rigour as critical numbers and priorities.
...
- **Never** – regularly acts in ways that conflict with this value

The consequence framework:

- **Always** – recognized publicly, considered for increased responsibility
- **Sometimes** – coaching plan developed, specific behaviour change identified, reviewed again in 90 days
- **Never** – formal performance conversation initiated immediately, 90 days to correct, exit if unchanged

A high performer who consistently scores Never on any value is not a high performer. Performance and values are evaluated together, not separately.

Ask the founder to confirm this feels right before moving to personal modelling.

### Personal modelling

For each confirmed value, present a specific behaviour the founder can demonstrate visibly so the team sees the values as operational, not aspirational. Ask the founder to confirm each one before closing.
...
Title: "Core Values – [Company Name] – [Date]"

The document has seven sections:

1. **Confirmed values in priority order** — each with its one-sentence statement
2. **Always/sometimes/never scenarios** for each value, presented as tables with colored headers (green for Always, yellow for Sometimes, red for Never) and white body rows. Use fictional names only.
...
6. **Scorecard review framework** — the three-tier rating scale with definitions and consequences
7. **Personal modelling behaviours** for each value
...
Present all sections inline, then produce the Word document for download. Close with:

"These are first drafts. Download the Word document and review it with your coach in the format they prefer, such as Mural or a similar visual workspace. If you have a co-founder, have them complete this exercise independently and use the synthesis section below to reconcile. Do not share your output with them before they complete theirs. Anchoring to your version defeats the purpose. The disagreements are the most valuable part. Once you and your coach have finalized the tools, add them to Metronome Software."
...
*7. Reconcile any differences in hiring questions, team rhythms, scorecard approaches, and personal modelling*
